...
Repo history unrelated to app. (Repo name and .docker-config/ predate the podman conversion; there is no
...
- .docker-config/ — quadlet/ (unit templates, worktree + proxy), Dockerfile, Dockerfile.playwright,  
...
- .units/ — (git-ignored) <slug>.env per worktree, generated by units:env: every value the unit
  templates interpolate. Outside the worktree on purpose — a worktree is a checkout of the *app*
...
Overrides go in .units/<slug>.share.env, which rails@ loads *after* .units/<slug>.env — last wins both
...
## Worktree stack (.docker-config/quadlet/, installed by units:install)
...
- Quadlet does NOT expand ${VAR} itself. systemd does, from the service environment — so every file
  whose keys use ${VAR} loads .units/<slug>.env under **[Service]** as well as [Container]. A
...
## Proxy (.docker-config/quadlet/, proxy.network + traefik/dozzle/home)
...
- nvim(v), claude(ai)/claude:newterm(ai:newterm)/claude:rebuild, db:dump/db:dump:clear, tags, proxy:*,
  clean. db:dump writes .docker-config/db-dumps/, which is also the restore path: db@'s initdb hook
  ...
  worktree's .units/<slug>.env references — a worktree missing that file disables the image sweep
...
- Dockerfile per service: if a custom image is needed, use .docker-config/Dockerfile.<service> and add a
